...
using Cacher = MedRecProImportClass.Helpers.PerformanceHelper;
using Newtonsoft.Json;

namespace MedRecProImportClass.DataAccess
{
    /// <summary>
    /// A generic repository for performing basic CRUD operations on database entities
    /// corresponding to the data models defined in MedRecProImportClass.DataModels.
    ...
    public class Repository<T> where T : class
    {
        ...
        public virtual async Task<List<DocumentDto>> GetCompleteLabelsAsync(int? pageNumber, int? pageSize)
        {
            ...
            // DtoLabelAccess is not included in the import library - this method is for querying
            // The import library only needs to write data, not build DTO projections
            throw new NotSupportedException("GetCompleteLabelsAsync is not supported in the import library. Use the full MedRecPro project for DTO queries.");
        ...
        /// <returns>A list containing the complete, hierarchical Document label if found, empty list otherwise.</returns>
        /// <example>
        /// <code>
        /// var documentGuid = Guid.Parse("12345678-1234-1234-1234-123456789012");
        /// var documents = await repository.GetCompleteLabelsAsync(documentGuid);
        /// </code>
        /// </example>
        /// <remarks>
