...
namespace MedRecProTest.Architecture
{
    ...
    public class AeRiskViewSqlTests
    {
        ...
        /**************************************************************/
        /// <summary>
        /// Reads a repository file from the build-time repository marker.
        /// </summary>
        /// <param name="relativePath">Path segments relative to the shared repository parent.</param>
        /// <returns>The file text.</returns>
        /// <remarks>
        /// Source lookup is independent of the compiled output directory.
        /// </remarks>
        ...
        private static string readRepoFile(params string[] relativePath)
        {
            ...
            var candidate = Path.Combine(RepositorySourceRoot.RootPath, Path.Combine(relativePath));
            if (File.Exists(candidate))
            {
                return File.ReadAllText(candidate);
            }

            Assert.Fail($"Unable to locate repository source file '{candidate}'.");
            return string.Empty;
