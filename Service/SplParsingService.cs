...
namespace MedRecPro.Service
{
    ...
    public class SplXmlParser
    {
        ...
        private static readonly XNamespace ns = c.XML_NAMESPACE; // Default HL7 v3 namespace
        ...
        public async Task<SplFileImportResult> ParseAndSaveSplDataAsync(string xmlContent, string fileNameInZip)
        {
            ...
            try
            {
                ...
                // 1. Parse the root <document> element to establish context.
                if (_sectionParsers.TryGetValue("document", out var documentParser))
                {
                    ...
                    {
                        // If the root document fails to parse, we cannot continue.
                ...
                // 2. Parse the <author> element.
                var authorEl = docEl.Element(ns + "author");
                if (authorEl != null && _sectionParsers.TryGetValue("author", out var authorParser))
                {
                    var authorParseResult = await authorParser.ParseAsync(authorEl, context);
                    context.UpdateFileResult(authorParseResult);
                }

                // 3. Parse the <structuredBody> element.
                var structuredBodyEl = docEl.Element(ns + "component")?.Element(ns + "structuredBody");
                if (structuredBodyEl != null && _sectionParsers.TryGetValue("structuredbody", out var structuredBodyParser))
                {
