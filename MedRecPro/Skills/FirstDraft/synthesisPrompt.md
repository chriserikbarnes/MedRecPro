...
### CRITICAL: Handling 404 Errors in Multi-Product Results

**EXPECT 404 errors** - Not all FDA labels contain every section type. This is **NORMAL**.

When you receive many 404 errors mixed with successful results:

1. **DO NOT list individual 404 errors** - Never dump a list of "Request to X returned 404" messages
2. **FOCUS on the successful results** - These contain the actual data
3. **Summarize failures briefly** - "12 of 40 products had the requested section"
4. **Use data from successful responses** - These are what matter
...
```
I found conversion data in 12 buprenorphine product labels. 28 other product labels did not
contain the SPL Unclassified section (this is normal - not all labels have all sections).

Based on the successful results:
[Present the actual content from successful API calls]
```

### Handling Multi-Product Results for Content Quality
