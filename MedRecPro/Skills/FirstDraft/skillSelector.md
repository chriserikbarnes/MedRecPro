...
Use for queries about:
- Opioid dose conversions (morphine to hydromorphone, fentanyl to morphine, methadone to buprenorphine, etc.)
- Equianalgesic dosing tables and ratios
- Opioid-tolerant patient definitions
- Morphine milligram equivalents (MME)
- Switching between opioid medications

**Keywords**: equianalgesic, opioid conversion, convert morphine, convert hydromorphone, convert fentanyl, convert oxycodone, convert methadone, convert buprenorphine, morphine equivalent, mme, opioid tolerant, dose conversion, switching opioids, opioid switch, equivalent dose, morphine to hydromorphone, hydromorphone to morphine, fentanyl to morphine, morphine to fentanyl, oxycodone to morphine, morphine to oxycodone, methadone, buprenorphine, methadone to buprenorphine, buprenorphine to methadone, opioid, conversion from, conversion to

**CRITICAL WORKFLOW - 6 Steps for BOTH Opioids, BOTH Sections**:

1. **Search SOURCE opioid products**: Call `/api/Label/ingredient/advanced?substanceNameSearch={sourceOpioid}&pageSize=5`
   - Use `outputMapping: { "sourceDocumentGuids": "documentGUID[]" }` (the `[]` suffix is REQUIRED)

2. **Search TARGET opioid products**: Call `/api/Label/ingredient/advanced?substanceNameSearch={targetOpioid}&pageSize=5`
   - Use `outputMapping: { "targetDocumentGuids": "documentGUID[]" }` (the `[]` suffix is REQUIRED)

3. **Get Dosage from SOURCE**: Call `/api/Label/markdown/sections/{{sourceDocumentGuids}}?sectionCode=34068-7`
   - Fetches Dosage and Administration (main header) from ALL source opioid products

4. **Get Unclassified from SOURCE**: Call `/api/Label/markdown/sections/{{sourceDocumentGuids}}?sectionCode=42229-5`
   - **CRITICAL: "Table 1: Conversion Factors" is in this section (42229-5)**
   - Contains detailed conversion tables and calculation examples

5. **Get Dosage from TARGET**: Call `/api/Label/markdown/sections/{{targetDocumentGuids}}?sectionCode=34068-7`
   - Fetches Dosage and Administration from ALL target opioid products

6. **Get Unclassified from TARGET**: Call `/api/Label/markdown/sections/{{targetDocumentGuids}}?sectionCode=42229-5`
   - Contains detailed conversion tables from target opioid labels

**ONLY USE THESE SECTION CODES**:
- **34068-7** (Dosage and Administration) - Main section header
- **42229-5** (SPL Unclassified) - Subsections with detailed conversion tables like "Table 1: Conversion Factors"
...
- Detailed conversion tables (e.g., methadone's "Table 1") are in section **42229-5**
...
| "Convert methadone to buprenorphine" | equianalgesicConversion |
...
| "What is the conversion from methadone to buprenorphine" | equianalgesicConversion |
