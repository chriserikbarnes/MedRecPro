<#
.SYNOPSIS
    MedRecPro Orange Book Migration Script using BCP (Bulk Copy Program)

.DESCRIPTION
    This script facilitates bulk data transfer of FDA Orange Book tables from a local
    SQL Server instance to Azure SQL Database using the BCP utility. These tables are
    populated locally by the OrangeBookImporter console module to avoid expensive vCore
    consumption on Azure.

    The script operates in two distinct phases:
    1. EXPORT: Extracts Orange Book table data from local SQL Server to native binary files (.dat)
    2. IMPORT: Truncates Azure target tables in dependency order, then loads data files

    Tables Handled (8 total):

    Lookup Table:
    - OrangeBookPatentUseCode                  (~4,400 rows)  - Patent use code definitions

    Core Tables:
    - OrangeBookApplicant                      (~2K+ rows)    - Applicant company lookup
    - OrangeBookProduct                        (~47K+ rows)   - Central product fact table
    - OrangeBookPatent                         (~16K+ rows)   - Patent records per product
    - OrangeBookExclusivity                    (~2K+ rows)    - Exclusivity periods per product

    Junction Tables:
    - OrangeBookProductMarketingCategory       - Product <-> MarketingCategory
    - OrangeBookProductIngredientSubstance     - Product <-> IngredientSubstance
    - OrangeBookApplicantOrganization          - Applicant <-> Organization

    Key Features:
    - Dependency-aware truncation order (junctions first, parents last)
    - Dependency-aware import order (parents first, junctions last)
    - Identity value preservation (-E flag) for surrogate key consistency
...
.PARAMETER DataPath
    Directory for storing exported .dat files (default: C:\MedRecPro-Migration\OrangeBook)
...
.EXAMPLE
    # Export only
    .\MedRecPro-OrangeBook-Export-Import.ps1 -Operation Export

.EXAMPLE
    # Import only (after export completed on another machine)
    .\MedRecPro-OrangeBook-Export-Import.ps1 -Operation Import -AzureServer "myserver.database.windows.net" -AzureDatabase "MedRecPro" -AzureUser "admin"

.EXAMPLE
    # Full migration
    .\MedRecPro-OrangeBook-Export-Import.ps1 -Operation Both -AzureServer "myserver.database.windows.net" -AzureDatabase "MedRecPro" -AzureUser "admin"
...
.NOTES
    ...
    Requires:
        ...
        - Orange Book tables must already exist on Azure
          (run MedRecPro-TableCreate-OrangeBook.sql on Azure first)

    Prerequisites:
        - Run the Orange Book import module locally first to populate the tables
...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$DataPath = "C:\MedRecPro-Migration\OrangeBook",
...
# Orange Book tables - import/export order (parents first, children/junctions last)
# This ordering ensures parent tables are populated before dependent tables
$Tables = @(
    "OrangeBookPatentUseCode"                    # Standalone lookup (no dependencies)
    "OrangeBookApplicant"                         # Parent: referenced by Product & ApplicantOrganization
    "OrangeBookProduct"                           # Parent: referenced by Patent, Exclusivity, junctions
    "OrangeBookPatent"                            # Child of Product
    "OrangeBookExclusivity"                       # Child of Product
    "OrangeBookProductMarketingCategory"           # Junction: Product <-> MarketingCategory
    "OrangeBookProductIngredientSubstance"         # Junction: Product <-> IngredientSubstance
    "OrangeBookApplicantOrganization"              # Junction: Applicant <-> Organization
)

# Truncation order: reverse of import order (junctions/children first, parents last)
# Ensures dependent data is cleared before parent data
$TruncationOrder = @(
    "OrangeBookApplicantOrganization"              # Junction (depends on Applicant)
    "OrangeBookProductIngredientSubstance"          # Junction (depends on Product)
    "OrangeBookProductMarketingCategory"             # Junction (depends on Product)
    "OrangeBookExclusivity"                          # Child (depends on Product)
    "OrangeBookPatent"                               # Child (depends on Product)
    "OrangeBookPatentUseCode"                        # Standalone lookup
    "OrangeBookProduct"                              # Parent (depends on Applicant)
    "OrangeBookApplicant"                            # Root parent
)
...
function Write-LogMessage {
    ...
    # Append to log file
    $logFile = Join-Path $DataPath "orangebook-migration-log.txt"
...
function Invoke-AzureTruncate {
    ...
    .DESCRIPTION
        Executes TRUNCATE TABLE for each Orange Book table on the Azure target.
    ...
    Write-Host "Truncating target tables on Azure (dependency order)..." -ForegroundColor Cyan
    $allSuccess = $true
...
function Invoke-ExportPhase {
    <#
    .SYNOPSIS
        Exports Orange Book tables from local SQL Server to native binary files

    .DESCRIPTION
        Iterates through the configured Orange Book tables and exports each to a .dat file
    ...
    Write-Host "║           EXPORT PHASE - LOCAL ORANGE BOOK TABLES               ║" -ForegroundColor Cyan
    Write-Host "╚══════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
    ...
    foreach ($table in $Tables) {
        ...
        Write-Progress -Activity "Exporting Orange Book Tables" `
    ...
    Write-Progress -Activity "Exporting Orange Book Tables" -Completed
    ...
    $inventoryCsv = Join-Path $DataPath "orangebook-export-inventory.csv"
...
function Invoke-ImportPhase {
    <#
    .SYNOPSIS
        Imports all exported Orange Book data files to Azure SQL Database in parallel

    .DESCRIPTION
        Truncates target tables on Azure in dependency order (junctions first, parents last),
        then loads .dat files using parallel BCP operations with identity preservation (-E).
    ...
    Write-Host ""
    Write-Host "╔══════════════════════════════════════════════════════════════════╗" -ForegroundColor Yellow
    Write-Host "║          IMPORT PHASE - AZURE SQL (ORANGE BOOK)                 ║" -ForegroundColor Yellow
    Write-Host "╚══════════════════════════════════════════════════════════════════╝" -ForegroundColor Yellow
    Write-Host ""
    ...
    # Sort inventory into dependency order (parents first)
    $tableOrder = @{}
    for ($i = 0; $i -lt $Tables.Count; $i++) {
        $tableOrder[$Tables[$i]] = $i
    }
    $Inventory = @($Inventory | Sort-Object {
        if ($tableOrder.ContainsKey($_.Table)) { $tableOrder[$_.Table] } else { 999 }
    })
    ...
    Write-Host "Tables queued for import (dependency order):" -ForegroundColor White
    ...
    Write-Host "║  Orange Book tables on Azure SQL. This is a full refresh.        ║" -ForegroundColor Red
    Write-Host "║                                                                  ║" -ForegroundColor Red
    Write-Host "║  Truncation order respects logical dependencies:                 ║" -ForegroundColor Red
    Write-Host "║  junctions -> children -> lookups -> parents                     ║" -ForegroundColor Red
    Write-Host "║                                                                  ║" -ForegroundColor Red
    ...
    # Truncate target tables on Azure before importing (using dependency-aware order)
    ...
    if ($useParallel) {
        ...
        foreach ($item in $tablesToImport) {
            ...
            Write-Progress -Activity "Importing Orange Book Tables" `
        ...
        Write-Progress -Activity "Importing Orange Book Tables" -Completed
    }
    ...
    if ($importResults.Count -gt 0) {
        $resultsCsv = Join-Path $DataPath "orangebook-import-results.csv"
...
Write-Host "║                                                                  ║" -ForegroundColor Magenta
Write-Host "║        MedRecPro Orange Book Migration Utility v1.0              ║" -ForegroundColor Magenta
Write-Host "║                                                                  ║" -ForegroundColor Magenta
Write-Host "║     Bulk Copy Program — FDA Orange Book Data Transfer            ║" -ForegroundColor Magenta
Write-Host "║                                                                  ║" -ForegroundColor Magenta
Write-Host "╚══════════════════════════════════════════════════════════════════╝" -ForegroundColor Magenta
Write-Host ""
...
# Display tables being handled (grouped by category)
Write-Host "Target tables (import order):" -ForegroundColor White
Write-Host "  Lookups:" -ForegroundColor Gray
Write-Host "    - OrangeBookPatentUseCode" -ForegroundColor Gray
Write-Host "  Parents:" -ForegroundColor Gray
Write-Host "    - OrangeBookApplicant" -ForegroundColor Gray
Write-Host "    - OrangeBookProduct" -ForegroundColor Gray
Write-Host "  Children:" -ForegroundColor Gray
Write-Host "    - OrangeBookPatent" -ForegroundColor Gray
Write-Host "    - OrangeBookExclusivity" -ForegroundColor Gray
Write-Host "  Junctions:" -ForegroundColor Gray
Write-Host "    - OrangeBookProductMarketingCategory" -ForegroundColor Gray
Write-Host "    - OrangeBookProductIngredientSubstance" -ForegroundColor Gray
Write-Host "    - OrangeBookApplicantOrganization" -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-LogMessage "Orange Book migration script completed." -Level Info
Write-Host ""
