...
<xsl:transform version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:v3="urn:hl7-org:v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" exclude-result-prefixes="v3 xsl">
		...
		<xsl:if test="v3:code and v3:name/text()[string-length(.) > 0]">
			...
			<tr>
				...
				<td class="formItem" style="width: 12%;">
					<xsl:value-of select="v3:assigningTerritory/v3:code/@displayName"></xsl:value-of>
				</td>
				<td class="formItem" style="width: 12%;">
					<xsl:value-of select="v3:assigningOrganization/v3:name"></xsl:value-of>
				</td>
				<td class="formItem" style="width: 15%;">
					<xsl:apply-templates select="v3:subjectOf/v3:document" mode="substance"></xsl:apply-templates>
				</td>
				<td class="formItem" style="width: 8%;">
					<xsl:value-of select="v3:subjectOf/v3:document/v3:bibliographicReferenceText/text()"></xsl:value-of>
				</td>
				<td class="formItem" style="width: 8%;">
					<xsl:value-of select="v3:subjectOf/v3:policy/v3:code/@code"></xsl:value-of>
				</td>
			</tr>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="v3:subjectOf/v3:document" mode="substance">
		<xsl:choose>
					...
					<li style="disc">
						<xsl:element name="a">
							<xsl:attribute name="href">
								<xsl:value-of select="v3:text/v3:reference/@value"></xsl:value-of>
							</xsl:attribute>
							<xsl:choose>
								<xsl:when test="v3:title/text()[string-length(.) > 0]">
									<xsl:value-of select="v3:title"></xsl:value-of>
								</xsl:when>
			...
			<xsl:when test="v3:text and v3:text/v3:reference/@value='' and v3:title/text()[string-length(.) > 0]">
				<ul>
					<li style="disc">
						<xsl:value-of select="v3:title"></xsl:value-of>
					</li>
				...
				<table width="100%" cellpadding="5" cellspacing="0" class="formTablePetite">
					...
					<tr>
						<td colspan="4" class="formHeadingTitle">
							...
							<xsl:for-each select="v3:code/@displayName">
								(<xsl:value-of select="."/>)
							</xsl:for-each>
							<xsl:for-each select="v3:partMoiety/v3:id/@extension">
					...
					<tr class="formTableRowAlt">
						...
						<td class="formItem">
							<xsl:choose>
								...
								<xsl:otherwise>
									<xsl:value-of select="v3:quantity/v3:numerator/v3:low/@value"/>-<xsl:value-of select="v3:quantity/v3:numerator/v3:high/@value"/>&#160;<xsl:if test="normalize-space(v3:quantity/v3:numerator/@unit)!='1'">
	...
	<xsl:template  mode="substance" match="v3:bond[v3:distalMoiety/v3:id/@extension]">
		<tr class="formTableRowAlt">
			...
			<td>
				<xsl:value-of select="v3:positionNumber[position() = 1]/@value"></xsl:value-of>
				<xsl:text>,</xsl:text>
				<xsl:value-of select="v3:positionNumber[position() = last()]/@value"></xsl:value-of>
			</td>
		</tr>
	</xsl:template>
				...
				<xsl:choose>
					...
					<xsl:when test="v3:value[@xsi:type='CV']">
						...
						<xsl:if test="v3:value/@code and not(v3:value/@displayName)">
							...
							<xsl:for-each select="v3:value/@codeSystem">
