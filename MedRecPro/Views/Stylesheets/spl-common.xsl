...
--><xsl:transform version="1.0"
				...
				<table cellpadding="0" class="tableContentAlign3911">
					<tr>
						...
						<td style="color: black;padding-left: 5px;text-align:right; font-family: Century Gothic,sans-serif;">
							<p>
								Form Approved: OMB No.xxxx-xxxx<br/>
		...
		<html>
			...
			<body class="spl" id="spl">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<h1>
							<xsl:apply-templates mode="mixed" select="v3:title"/>
						</h1>
					</xsl:otherwise>
				</xsl:choose>
				<div class="Contents">
					...
					<div class="DataElementsTables">
						...
						<xsl:if test="/v3:document/v3:code/@code = '77648-4'">
							<xsl:for-each select="v3:component/v3:structuredBody/v3:component[1]/v3:section/v3:subject/v3:manufacturedProduct[v3:manufacturedProduct[v3:name]]">
								<b>
									<xsl:value-of select="concat('Old ', ../../v3:code/@displayName,': ')"/>
								</b>
							...
							<xsl:for-each select="v3:component/v3:structuredBody/v3:component[2]/v3:section/v3:subject/v3:manufacturedProduct[v3:manufacturedProduct[v3:name]]">
								<b>
									<xsl:value-of select="concat('New ', ../../v3:code/@displayName,': ')"/>
								</b>
								<xsl:apply-templates mode="subjects" select="."/>
							</xsl:for-each>
						</xsl:if>
				...
				<xsl:if test="$root/v3:document/v3:code/@code = '82351-8'">
					<xsl:for-each select="v3:relatedDocument[@typeCode = 'SUBJ']">
						<b>
							...
							<a href="{concat('../', ., '.view')}">
								<xsl:value-of select="v3:relatedDocument/v3:setId/@root"/>
							</a>
							<br/>
						</b>
					</xsl:for-each>
						...
						<table width="100%" class="formTablePetite">
							...
							<xsl:for-each select="../v3:subjectOf/v3:approval[v3:code/v3:originalText]">
								<tr>
									...
									<td class="formItem">
										<xsl:value-of select="v3:code/v3:originalText"/>
									</td>
								</tr>
							</xsl:for-each>
						</table>
					...
					<td>
						<table width="100%" cellpadding="3" cellspacing="0" class="formTablePetite">
							...
							<xsl:for-each select="v3:participation[@typeCode='SBJ']/v3:role">
								<tr>
									<td class="formItem">
										<xsl:value-of select="v3:player/v3:name"/>
									</td>
									<td class="formItem">
										<xsl:value-of select="v3:scoper/v3:code/@code"/>
									</td>
								</tr>
							</xsl:for-each>
						</table>
					...
					<td>
						<table width="100%" cellpadding="3" cellspacing="0" class="formTablePetite">
							...
							<xsl:for-each select="v3:outboundRelationship[@typeCode='OBJT']/v3:targetAct/v3:participation[@typeCode='SBJ']">
								<tr>
									<td class="formItem">
										<xsl:value-of select="v3:role/v3:player/v3:name"/>
									</td>
									<td class="formItem">
			...
			<td class="formItem">
				<xsl:choose>
					<xsl:when test="number(v3:sequenceNumber/@value) &lt; number(ancestor::v3:componentOf[1]/v3:sequenceNumber/@value)">
						<xsl:for-each select="ancestor::v3:componentOf[1]/v3:pauseQuantity[@value and @unit][1]">
							<!-- remember that the before case is different from during and after, see  -->
	...
	<!-- REMS templates end -->
	<!-- helper templates -->
	<xsl:template priority="2" match="v3:highlight//@width[not(contains(.,'%'))]" />
	<!-- This would avoid things moving out of 2-column view -->
		...
		<table>
			...
			<tr>
				...
				<td>
					...
					<xsl:if test="string-length(./v3:state)>0">
						,&#160;<xsl:value-of select="./v3:state"/>
					</xsl:if>
					<xsl:if test="string-length(./v3:postalCode)>0">
				...
				<xsl:if test="v3:excerpt and not($suppressTitle)">
					<xsl:call-template name="styleCodeAttr">
						...
						<xsl:with-param name="additionalStyleCode">
							Highlight<xsl:if test="ancestor::v3:section[v3:excerpt]">Sub</xsl:if>Section
						</xsl:with-param>
					</xsl:call-template>
				</xsl:if>
	...
	<xsl:template mode="highlights" match="v3:section[v3:code[@codeSystem='2.16.840.1.113883.6.1' and @code='34066-1']][v3:excerpt]">
		<!-- BOXED WARNING -->
		<xsl:param name="doNotSuppressFrontOrBackMatter" select="/.."/>
...
token.
		...
		<div class="DocumentTitle">
			<p class="DocumentTitle">
				...
				<xsl:for-each select=".//v3:subject[not(/v3:document/v3:code/@code = '77648-4')][v3:manufacturedProduct[v3:manufacturedProduct|v3:manufacturedMedicine]]">
					...
					<xsl:variable name="curProductHeaderString">
						<xsl:call-template name="headerString">
							<xsl:with-param name="curProduct" select="v3:manufacturedProduct/*[self::v3:manufacturedProduct or self::v3:manufacturedMedicine]" />
						</xsl:call-template>
					</xsl:variable>
					<xsl:choose>
			...
			<xsl:for-each select="$disclaimers/document[code/@code = $root/v3:document/v3:code/@code]/disclaimer[code/@code = $marketingCategories/@code]/text">
				<p class="disclaimer">
					Disclaimer: <xsl:copy-of select="node()"/>
				</p>
			</xsl:for-each>
			...
			<xsl:if test="/v3:document[v3:code/@code = '82351-8']">
				<b>
					<xsl:value-of select="substring-before(/v3:document/v3:title,'Document')"/>
					<br/>
	...
	<xsl:template name="titleNumerator">
		<xsl:for-each
			...
			<xsl:if test="./v3:quantity/v3:numerator/@unit">
			...
			<xsl:if test="$revisionTime">
				...
				<div class="DocumentMetadata">
					...
					<div style="display:none">
						<xsl:attribute name="id">
							<xsl:text>documentTypeCode</xsl:text>
						</xsl:attribute>
		...
		<xsl:if test="not($root/v3:document/v3:code/@code = '3565717') and not($root/v3:document/v3:code/@code = '3565715')">
							...
							<td>
								<table width="100%" cellpadding="3" cellspacing="0" class="formTablePetite">
									<tr class="formTableRowAlt">
										...
										<td class="formItem">
											<xsl:value-of select="../v3:subjectOf/v3:document/v3:bibliographicDesignationText"/>
										</td>
									</tr>
								</table>
							</td>
			...
			<xsl:if test="$root/v3:document/v3:code/@code = '58476-3' and ../v3:subjectOf/v3:characteristic">
				<table cellpadding="3" cellspacing="0" style="border:2px solid">
					<tr>
						<td colspan="3">
							<span style="font-size: 125%;font-weight: bold;">Supplement Facts</span>
						</td>
					</tr>
					<tr style="border-bottom:2px solid">
						...
						<td style="font-size: 100%;font-weight: bold;">
							<span style=" float: left;">
								Serving Size : <xsl:value-of select="$char[../v3:code/@code = '101.9(b)/1']/v3:translation/@value|$char[../v3:code/@code = '101.9(b)/2']/v3:translation/@value"/><xsl:text> </xsl:text><xsl:value-of select="$char[../v3:code/@code = '101.9(b)/1']/v3:translation/@displayName|$char[../v3:code/@code = '101.9(b)/2']/v3:translation/@displayName"/>
							</span>
						</td>
						<td></td>
						<td style="font-size: 100%;font-weight: bold;">
							<span style=" float: left;">
								Serving per Container : <xsl:value-of select="$char[../v3:code/@code = '101.9(b)(8)']/@value"/>
							</span>
						</td>
					</tr>
					<tr style="border-bottom:2px solid">
						...
						<tr>
							<th align="left" class="formHeadingTitle">
								<xsl:choose>
									<xsl:when test="v3:ingredient">
										<strong>
											<xsl:text>Abstract Product Concept</xsl:text>
										</strong>
									</xsl:when>
									<xsl:otherwise>
										<strong>
											<xsl:text>Application Product Concept</xsl:text>
										</strong>
									</xsl:otherwise>
								</xsl:choose>
							</th>
		...
		<tr>
			<td>
				<table width="100%" cellspacing="0" cellpadding="5">
					<tr>
						<td class="contentTableTitle">
							Part <xsl:value-of select="count(../preceding-sibling::v3:part)+1"/> of <xsl:value-of select="count(../../v3:part)"/>
						</td>
					</tr>
					<xsl:call-template name="piMedNames"/>
				</table>
				...
				<tr>
					...
					<xsl:for-each select="(v3:ingredientSubstance|v3:activeIngredientSubstance)[1]">
						<td class="formItem">
							...
							<xsl:for-each select="../v3:subjectOf/v3:substanceSpecification/v3:code[@codeSystem = '2.16.840.1.113883.3.6277']">
								<xsl:text> (Biological Drug Substance: </xsl:text>
								<xsl:value-of select="@displayName"/>
								<xsl:text>, </xsl:text>
								<xsl:value-of select="@code"/>
								<xsl:text>)</xsl:text>
					...
					<td class="formItem">
						<xsl:value-of select="v3:quantity/v3:numerator/@value"/>&#160;<xsl:if test="normalize-space(v3:quantity/v3:numerator/@unit)!='1'">
							<xsl:value-of select="v3:quantity/v3:numerator/@unit"/>
						</xsl:if>
						<xsl:if test="(v3:quantity/v3:denominator/@value and normalize-space(v3:quantity/v3:denominator/@value)!='1') 
							...
							&#160;in&#160;<xsl:value-of select="v3:quantity/v3:denominator/@value"
													/>&#160;<xsl:if test="normalize-space(v3:quantity/v3:denominator/@unit)!='1'">
								<xsl:value-of select="v3:quantity/v3:denominator/@unit"/>
							</xsl:if>
						</xsl:if>
					</td>
			...
			<xsl:for-each select="v3:ingredient[@classCode='IACT']|v3:inactiveIngredient">
				<tr>
					...
					<td class="formItem">
						...
						<xsl:if test="v3:quantity/v3:denominator/@value and normalize-space(v3:quantity/v3:denominator/@unit)!='1'">
							&#160;in&#160;<xsl:value-of select="v3:quantity/v3:denominator/@value"
						/>&#160;<xsl:if test="normalize-space(v3:quantity/v3:denominator/@unit)!='1'">
								<xsl:value-of select="v3:quantity/v3:denominator/@unit"/>
							</xsl:if>
					...
					<xsl:for-each select="(v3:ingredientSubstance|v3:activeIngredientSubstance)[1]">
						<td class="formItem">
							...
							<xsl:if test="normalize-space(v3:ingredientSubstance/v3:activeMoiety/v3:activeMoiety/v3:name)">
								(<xsl:value-of
							select="v3:ingredientSubstance/v3:activeMoiety/v3:activeMoiety/v3:name"/>)
							</xsl:if>
						</td>
					</xsl:for-each>
					<td class="formItem">
	...
	<xsl:template name="color">
		...
		<td class="formItem">
			<xsl:for-each select="$path/v3:value">
				<xsl:if test="position() > 1">,&#160;</xsl:if>
				<xsl:value-of select="./@displayName"/>
				<xsl:if test="normalize-space(./v3:originalText)">
	...
	<xsl:template name="shape">
		...
		<td class="formItem">
			...
			<xsl:if test="normalize-space($path/v3:value/v3:originalText)">
				(<xsl:value-of select="$path/v3:value/v3:originalText"/>)
			</xsl:if>
		</td>
	</xsl:template>
	...
	<xsl:template mode="ldd" match="v3:instanceOfKind/v3:productInstance" priority="1" name="next-match-fill-lot">
		...
		<tr>
			<td rowspan="{count(v3:ingredient)+1}" class="formItem">
				<xsl:value-of select="v3:id/@extension"/>
			</td>
		</tr>
	...
	<xsl:template mode="ldd" name="bulk" match="v3:productInstance/v3:ingredient/v3:ingredientProductInstance" priority="1">
		<tr>
			...
			<td colspan="5" class="formItem">
				<xsl:value-of select="v3:asInstanceOfKind/v3:kindOfMaterialKind/v3:name"/>
			</td>
			<td class="formItem">
				<xsl:value-of select="../v3:quantity/v3:numerator/@value"/>
			</td>
			...
			<td class="formItem">
				<xsl:value-of select="../v3:subjectOf/v3:productEvent[v3:code[@code='C43360' and @codeSystem='2.16.840.1.113883.3.26.1.1']]/v3:performer/v3:assignedEntity/v3:representedOrganization/v3:id/@extension"/>
			</td>
		</tr>
	</xsl:template>
	<!-- LABEL LOT -->
	...
	<xsl:template mode="ldd" match="v3:productInstance/v3:member/v3:memberProductInstance/v3:asContent/v3:container" priority="1" name="next-match-label-lot">
		<tr>
			...
			<td class="formItem">
				<xsl:value-of select="../v3:subjectOf/v3:productEvent[v3:code[@code = 'C106325' or @code = 'C106326']]/v3:code/@displayName"/>
			</td>
			<td class="formItem">
				<xsl:value-of select="../v3:subjectOf/v3:productEvent[v3:code[@code = 'C106325' or @code = 'C106326']]/v3:effectiveTime/v3:low/@value"/>
			</td>
			<td class="formItem">
				<xsl:value-of select="../../v3:expirationTime/v3:high/@value"/>
			</td>
			<td class="formItem">
			...
			<table width="100%" cellpadding="3" cellspacing="0" class="formTableMorePetite">
				<tr>
					<td colspan="4" class="formHeadingReg">
						<span class="formHeadingTitle" >Marketing Information</span>
					</td>
				</tr>
				<tr>
	...
	<xsl:template mode="subjects" match="//v3:author/v3:assignedEntity/v3:representedOrganization">
		<xsl:if test="(count(./v3:name)>0)">
			<table width="100%" cellpadding="3" cellspacing="0" class="formTableMorePetite">
				<tr>
					<td colspan="4" class="formHeadingReg">
						<span class="formHeadingTitle" >Labeler -&#160;</span>
						<xsl:value-of select="./v3:name"/>
						<xsl:choose>
		...
		<xsl:for-each select="v3:contactParty">
			...
			<tr class="formTableRowAlt">
				...
				<td class="formItem">
					...
					<div style="display:none">
						<xsl:attribute name="id">
							<xsl:text>contactMailId</xsl:text>
						</xsl:attribute>
						<xsl:value-of select=" substring-after(v3:telecom/@value[starts-with(.,'mailto:')][1], 'mailto:')"/>
					</div>
				</td>
									...
									<xsl:for-each select="$indicationSection//v3:excerpt/v3:highlight/v3:subject">
										<tr class="formTableRowAlt">
											<td class="formItem" valign="top">
												<strong>
													<xsl:value-of select="./v3:substanceAdministration/v3:reason/v3:indicationObservationCriterion/v3:value/@displayName"/> (<xsl:value-of
												select="./v3:substanceAdministration/v3:reason/v3:indicationObservationCriterion/v3:code/@displayName"/>)
												</strong>
											</td>
											<td class="formItem" valign="top">
	...
	<xsl:template mode="indication" match="v3:value[@xsi:type='IVL_PQ']">
		<xsl:choose>
			...
			<xsl:when test="v3:low and not(v3:high)">
				&#8805; <xsl:value-of select="v3:low/@value"/><xsl:value-of select="v3:low/@unit"/>
			</xsl:when>
			<xsl:when test="not(v3:low) and v3:high">
				&#8804;<xsl:value-of select="v3:high/@value"/><xsl:value-of select="v3:high/@unit"/>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
